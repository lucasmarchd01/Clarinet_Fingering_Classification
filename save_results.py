...
with mp_holistic.Holistic(
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            for i in range(len(pose_tubuh)):
                results.pose_landmarks.landmark[i].x = (
                    results.pose_landmarks.landmark[i].x * image.shape[0]
                )
                results.pose_landmarks.landmark[i].y = (
                    results.pose_landmarks.landmark[i].y * image.shape[1]
                )
                data_tubuh.update({pose_tubuh[i]: results.pose_landmarks.landmark[i]})
            alldata.append(data_tubuh)

        if results.right_hand_landmarks:
            data_tangan_kanan = {}
            for i in range(len(pose_tangan)):
                results.right_hand_landmarks.landmark[i].x = (
                    results.right_hand_landmarks.landmark[i].x * image.shape[0]
                )
                results.right_hand_landmarks.landmark[i].y = (
                    results.right_hand_landmarks.landmark[i].y * image.shape[1]
                )
                data_tubuh.update(
        ...
        if results.left_hand_landmarks:
            ...
            for i in range(len(pose_tangan)):
                results.left_hand_landmarks.landmark[i].x = (
                    results.left_hand_landmarks.landmark[i].x * image.shape[0]
                )
                results.left_hand_landmarks.landmark[i].y = (
                    results.left_hand_landmarks.landmark[i].y * image.shape[1]
                )
                data_tubuh.update(
        ...
        )  # sudah menampilkan backgrounnd hitam dan skeleton
